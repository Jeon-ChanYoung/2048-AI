...
class Alpha_Beta_Pruning:
    ...
    def add_tile(self, depth, alpha, beta):
        ...
        if n_empty >= 0 and depth >= 5:
            return self.evaluation(self.board, n_empty)
        if n_empty == 0:
            _, new_score = self.maximize(depth+1, alpha, beta)
        sum_score = 0
        ...
        for y,x in empty_cells:
            for num in [2, 4]:
                ...
                _, new_score = new_game.maximize(depth+1, alpha, beta)
    ...
    def maximize(self, depth=0, alpha=-np.inf, beta=np.inf):
        best_score = -np.inf
        best_action = None
        ...
        for action in range(4):
            ...
            new_score = new_game.add_tile(depth + 1, alpha, beta)
