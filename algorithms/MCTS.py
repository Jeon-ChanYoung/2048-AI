...
class MCTS:
    ...
    def maximize(self, iterations=250):
        root = Node(self.get_state(), None)
        
        with ThreadPoolExecutor() as executor:
            futures = [executor.submit(self.run_mcts_iteration, root) for _ in range(iterations)]
            for future in futures:
                future.result()  # Ensure all tasks are completed

        if not root.children:
            return random.choice([0, 1, 2, 3]), None  # Fallback to random action if no children

        best_action = max(root.children.values(), key=lambda n: n.visits).action
    ...
    def expansion(self, node):
        for action in range(4):
            if action not in node.children:
                ...
                new_node = Node(new_state, action, parent=node)
                node.children[action] = new_node

    def simulation(self, node):
        game_copy = deepcopy(self)
        game_copy.board = node.state[0]
...
class Node:
    ...
    def uct(self):
        ...
        for child in self.children.values():
            # Avoid division by zero by checking if visits > 0
            if child.visits > 0:
                uct_value = (child.value / child.visits +
                             np.sqrt(2 * np.log(self.visits + 1) / child.visits))
            else:
                # Assign a high UCT value if this child has not been visited
                uct_value = np.inf
